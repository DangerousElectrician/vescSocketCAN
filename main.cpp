...
int main( int argc, char **argv) {
	if(argc < 3) {
		std::cout << "remember to provide vesc id" << std::endl;
		return 0;
	}
	Vesc motor((char*) "can0", atoi(argv[1]), atoi(argv[2])); //TODO: this char* thing is a bit iffy, is this the right way to do it
	//motor.setCustom(0.1);
	//float current = 0;
	while(1) {
